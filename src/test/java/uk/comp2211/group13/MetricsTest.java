...
public class MetricsTest {
  ...
  public void bounceRateTest() {
    ingest();
    double result = 0;
    Assert.assertNotNull(metrics.bounceRatePage());
    Assert.assertEquals(BigDecimal.valueOf(result), BigDecimal.valueOf(metrics.bounceRatePage()));
  }
  ...
  public void totalCastTest() {
    ...
    Assert.assertNotNull(data.request().getImpressionCost() + data.request().getClickCost());
    Assert.assertEquals(BigDecimal.valueOf(result), BigDecimal.valueOf(data.request().getImpressionCost() + data.request().getClickCost()));
  }
}
